...
if __name__ == "__main__":
    ...
    model_AGMFNet = AGMFNet_ResNet50(channel=32).cuda()

    optimizer = torch.optim.Adam(model_AGMFNet.parameters(), opt.lr)
    LogitsBCE = torch.nn.BCEWithLogitsLoss()
    net, optimizer = amp.initialize(model_AGMFNet, optimizer, opt_level='O1') 
    ...
    for epoch_iter in range(1, opt.epoch):
        ...
        trainer(train_loader=train_loader, model=model_AGMFNetz,
